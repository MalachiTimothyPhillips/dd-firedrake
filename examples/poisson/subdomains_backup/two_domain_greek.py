...
par_loop( 'for ( int i=0; i < f.dofs; i++ ) f[i][0] = 1.0;', dx(Ω1), {'f': (I_O1, WRITE)} )
I_cg_O1 = Function(V)
...
par_loop( 'for ( int i=0; i < f.dofs; i++ ) f[i][0] = 1.;', dx(Ω2), {'f': (I_O2, WRITE)} )
I_cg_O2 = Function(V)
...
a1 = (dot(grad(u), grad(v))) * dx(Ω1)
L1 = f * v * dx(Ω1)
a2 = (dot(grad(v), grad(u))) * dx(Ω2)
L2 = f * v * dx(Ω2)
...
e1=u2
e2=u1
bc_dOmega1_n_Omega2=DirichletBC(V,e1,dΩ1nΩ2)
bc_dOmega2_n_Omega1=DirichletBC(V,e2,dΩ2nΩ1)
...
e1=assemble(dot(u1-f,u1-f)*dx(Ω1))
e2=assemble(dot(u2-f,u2-f)*dx(Ω2))
